...
# ============================================================
# T1_global_statique_298K -- RG=94.34 self-consistent fit
# Companion to T1_recovery_static_298K.py, but uses ONLY the night of
# 25-26/08 data (D1=0.02s to 400s, 15 points, all RG=94.34) -- avoids
# any RG cross-calibration with the archive.
#
# NS differs between blocks (128 vs 32) -- intensity is normalized to
# per-scan (intensity/NS) before fitting.
#
# exp381 (D1=0.5s) excluded: known DS=9 bug (copy-paste from exp380,
# under-equilibrated). D1=0.5s is covered anyway by exp394.
# ============================================================
...
ZF_FACTOR = 1
PEAK_PPM_WINDOW = (-10, 15)   # real line is always within -4..+8 ppm here; this
                              # window keeps a safe margin while excluding the
                              # noise region where argmax could mispick at very
                              # low signal (D1=0.02/0.05s)
PPM_OUTLIER_THRESHOLD = 5.0   # ppm from the median -- flags a likely mispicked noise peak (kept as a safety net)
OUTPUT_NAME = r"D:\Postdoc\Figures\T1_global_statique_298K"  # separate name -- compare before promoting
# ================================================
...
if __name__ == "__main__":
    # === STEP 1: process every point, normalize by NS, sanity-check peak position ===
    ...
    # Sanity check: intensity should be monotonically non-decreasing with D1
    # (a recovery curve can only rise or plateau, never fall).
    diffs = np.diff(I)
    if np.any(diffs < 0):
    ...
    # === STEP 2: fit -- biexponential and triexponential ===
    ...
    # NB: tick-mark spacing is handled inside export_agr itself -- no
