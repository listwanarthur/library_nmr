...
# ============================================================
# T1_global_statique_345K -- campagne a venir
# Meme structure/pipeline que T1_recovery_static_315K.py (biexp + triexp,
# force-include, leave-one-out/reweighting, verification plateau).
#
# D1=400s ET 500s inclus DES LE DEPART dans la grille (cf. 298K/330K/
# 360K/315K qui ont tous du rajouter des points apres coup pour fermer
# le plateau -- decision explicite de ne plus se faire avoir).
#
# A FAIRE AVANT DE LANCER :
#  1. Remplir EXP_START_SERIES1 (et EXP_START_SERIES2 si tu refais les
#     deux passes comme a 315K) avec le numero de la premiere experience
#     Bruker de la serie -- experiences supposees consecutives dans
#     l'ordre de _D1_LABELS ci-dessous (meme convention que 315K).
#  2. Verifier RG (rga fait juste avant de lancer -- ne PAS reutiliser une
#     valeur heritee d'une autre temperature/session sans le refaire).
#  3. Verifier PEAK_PPM_WINDOW sur le spectre a D1=500s (le mieux resolu)
#     avant de faire confiance a la fenetre par defaut (-10,15).
#  4. FORCE_INCLUDE_D1 est vide ci-dessous -- NORMAL, a remplir seulement
#     apres un premier run si des points sont mal repepres en ppm (verifie
#     que l'intensite brute du point exclu s'insere dans la tendance
#     monotone des voisins avant de le forcer).
# ============================================================

# === CONFIGURATION -- only section to edit ===
SERIES = "1"   # "1" ou "2" -- deux passes consecutives si tu veux verifier
                # la reproductibilite comme a 315K/330K (sinon laisse "1"
                # et ignore EXP_START_SERIES2)

EXP_START_SERIES1 = None   # <-- A REMPLIR (ex: 400)
EXP_START_SERIES2 = None   # <-- A REMPLIR seulement si SERIES="2" prevu

_D1_LABELS = [0.02, 0.05, 0.2, 0.35, 0.5, 0.7, 1, 1.5, 2, 3, 4, 6, 8,
              15, 30, 60, 120, 150, 250, 400, 500]
_NS_BY_D1 = {0.02: 128, 0.05: 128, 0.2: 128, 0.35: 128, 0.5: 128,
             0.7: 32, 1: 32, 1.5: 32, 2: 32, 3: 32, 4: 32, 6: 32, 8: 32,
             15: 32, 30: 32, 60: 32, 120: 32, 150: 32, 250: 32, 400: 32, 500: 32}
             # NS=32 partout comme a 315K -- reduis seulement 400/500
             # (ex: 8-16) si le temps spectro manque.

if SERIES == "1":
    if EXP_START_SERIES1 is None:
        raise ValueError("EXP_START_SERIES1 n'est pas rempli -- indique le numero "
                          "de la premiere experience Bruker de la serie 1 avant de lancer.")
    _EXP_LIST = list(range(EXP_START_SERIES1, EXP_START_SERIES1 + len(_D1_LABELS)))
else:
    if EXP_START_SERIES2 is None:
        raise ValueError("EXP_START_SERIES2 n'est pas rempli -- indique le numero "
                          "de la premiere experience Bruker de la serie 2 avant de lancer.")
    _EXP_LIST = list(range(EXP_START_SERIES2, EXP_START_SERIES2 + len(_D1_LABELS)))

DATA_DIR = r"D:\Postdoc\Datas\LLZO-400-sep26"   # <-- A VERIFIER/ADAPTER selon le dossier reel

# D1 (s): (path, NS)
DATASETS = {
    d1: (rf"{DATA_DIR}\{exp}", _NS_BY_D1[d1])
...
ZF_FACTOR = 1
PEAK_PPM_WINDOW = (-10, 15)     # A VERIFIER sur le spectre D1=500s avant de lancer serieusement
PPM_OUTLIER_THRESHOLD = 5.0
FORCE_INCLUDE_D1 = []            # a remplir apres le premier run (voir note en tete de fichier)
OUTPUT_NAME = rf"D:\Postdoc\Figures\T1_global_statique_345K_provisoire_serie{SERIES}"
# ================================================
...
if __name__ == "__main__":
    print(f"=== T1 series (static probe, 345K, serie {SERIES}) ===")
    records = []
    ...
    if bad:
        ...
        print("  -- verifie que l'intensite brute de ces points s'insere dans la tendance monotone des"
              " voisins avant de les rajouter via FORCE_INCLUDE_D1 (meme diagnostic qu'aux campagnes precedentes).")
    else:
    ...
    if use_tri:
        ...
        print("\nTriexponential relative residuals (%):", np.round(resid_tri, 2))
        print("  -- si un motif systematique subsiste en fin de courbe malgre D1=400/500s deja inclus,")
        print("     reprendre la logique 'variant A/B' developpee dans T1_recovery_static_315K.py")
        print("     (contraindre T1_fast ~biexp pour tester specifiquement une vraie queue lente).")

    USE_TRIEXP_FOR_FIGURE = False
    ...
    print(f"\nA D1={D1.max():.0f}s, on est a {fraction_plateau*100:.1f}% du plateau de recuperation "
          f"(biexp) -- {'OK' if fraction_plateau >= 0.95 else 'INSUFFISANT malgre D1=400/500s -- a investiguer'}.")
    ...
    ax.scatter(D1, I, color="blue", s=55, zorder=3, label="data (per-scan)")
    ...
    ax.set_title(rf"$^7$Li T$_1$ recovery — static probe, 345K (serie {SERIES})")
    ...
    agr_series = [
        dict(x=D1, y=I, mode="symbol", color="blue", legend="data (per-scan)"),
    ...
    export_agr(f"{OUTPUT_NAME}.agr", agr_series,
               ...
               xlog=True, title="7Li T1 recovery -- static probe, 345K")
