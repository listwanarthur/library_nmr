...
# ============================================================
# T1_global_statique_330K -- redo complet en un seul lot (28/08, exp900-920)
#
# La sonde ayant ete demontee/remontee entre-temps, toute la grille a ete
# reacquise dans UNE SEULE session continue (exp900-918 grille standard,
# exp919-920 extension a 400/500s) plutot que d'ajouter des points a
# l'ancienne grille (543-580) -- evite tout risque de biais lot-mixing
# (meme type de biais deja identifie sur la toute premiere tentative
# 330K). Audit acqus : RG=136.56 rigoureusement constant sur les 21
# points, aucune derive apres remontage.
#
# Objectif : fermer definitivement le plateau D1 (l'ancienne grille,
# D1max=250s, laissait une derive residuelle -- voir "plateau pas assez
# ferme" du 28/08). D1max=500s vise un rapport D1max/T1_slow ~19-20
# (vs ~10 avec 250s seul).
#
# Fit sur D1+AQ (pas D1 seul) -- lecon DS/AQ du 19-20/08.
# ============================================================
...
_NS_BY_D1 = {0.02: 128, 0.05: 128, 0.2: 128, 0.35: 128, 0.5: 128,
             ...
             400: 32, 500: 32}
_EXP_LIST = list(range(900, 921))   # 900 a 920 inclus, 21 points, un seul lot

# D1 (s): (path, NS)
DATASETS = {
    d1: (rf"D:\Postdoc\Datas\LLZO-400-aug26\{exp}", _NS_BY_D1[d1])
...
REFERENCE_SHIFT_PPM = 2      # inherited from the 298K referencing -- check the printed
                              # peak positions below; re-adjust here if they look shifted.
ZF_FACTOR = 1
PEAK_PPM_WINDOW = (-10, 15)   # same fix as 298K/330K precedent (26-27/08)
PPM_OUTLIER_THRESHOLD = 5.0   # ppm from the median -- flags a likely mispicked noise peak
FORCE_INCLUDE_D1 = [0.02, 0.35, 0.7, 1, 2, 4]         # leave empty on the first run -- check the
                              # EXCLUDED points printed at runtime, add a D1 here only if the raw
                              # intensity stays monotone despite the exclusion (wide/flat line ->
                              # ppm mispicked but intensity still trustworthy).
OUTPUT_NAME = r"D:\Postdoc\Figures\T1_global_statique_330K_provisoire_v2"
                              # _provisoire until you've checked below that the plateau is truly
                              # closed this time (tail residuals + leave-one-out stable) --
                              # rename without the suffix once confirmed.
# ================================================
...
if __name__ == "__main__":
    print("=== T1 series (static probe, 330K palier -- redo complet 900-920) ===")
    ...
    print("\n--- Biexponential fit robustness (leave-one-out from the tail / reweighting, t=D1+AQ) ---")
    for label, mask in [("tous", np.ones_like(D1, dtype=bool)),
                         ...
                         ("sans D1=150,250,400,500", ~np.isin(D1, [150, 250, 400, 500]))]:
        try:
            popt_i, pcov_i = curve_fit(biexp_recovery, D1_eff[mask], I[mask], p0=p0_bi, sigma=I[mask],
                                        maxfev=50000, bounds=bounds_bi)
            print(f"  {label:<24} T1_slow = {popt_i[3]:.3g} +/- {np.sqrt(np.diag(pcov_i))[3]:.2g} s")
        except RuntimeError:
    ...
    if use_tri:
        print("\n--- Triexponential fit robustness (leave-one-out from the tail / reweighting, t=D1+AQ) ---")
        for label, mask in [("tous", np.ones_like(D1, dtype=bool)),
                             ("sans D1=500", D1 != 500),
                             ("sans D1=400,500", ~np.isin(D1, [400, 500])),
                             ("sans D1=250,400,500", ~np.isin(D1, [250, 400, 500])),
            ...
            try:
                ...
                print(f"  {label:<24} T1c = {popt_i[5]:.3g} +/- {np.sqrt(np.diag(pcov_i))[5]:.2g} s")
            except RuntimeError:
                print(f"  {label:<24} fit failed")
        for label, sigma in [("non pondere", None), ("sigma=sqrt(I)", np.sqrt(I))]:
            try:
    ...
    print("-> regarde en particulier les residus des derniers points (D1=150/250/400/500) :")
    print("   s'ils se stabilisent/oscillent autour de 0 sans tendance soutenue, le plateau est ferme ;")
    print("   s'ils dérivent encore de façon monotone jusqu'au dernier point, ce n'est pas encore le cas")
    print("   (meme diagnostic que celui qui avait signale le probleme sur l'ancienne grille 543-580).")
    ...
    ax.scatter(D1, I, color="blue", s=55, zorder=3, label="data (per-scan, RG=136.56)")
    ...
    ax.set_title(r"$^7$Li T$_1$ recovery — static probe, 330K (redo complet 28/08, exp900-920, RG=136.56)")
    ...
    agr_series = [
        dict(x=D1, y=I, mode="symbol", color="blue", legend="data (per-scan, RG=136.56)"),
    ...
    export_agr(f"{OUTPUT_NAME}.agr", agr_series,
               ...
               xlog=True, title="7Li T1 recovery -- static probe, 330K (redo complet)")
