...
# ============================================================
# T1_global_statique_330K -- grille refaite entierement (27-28/08)
#
# Le resultat precedent (14 points, 26/08) melangeait deux lots
# d'acquisition sans le savoir (NS/RG differents) -- diagnostique via un
# ecart net (plusieurs dizaines de %) sur un point identique entre les
# deux lots (voir Check_lot_reproducibility.py, Bloc A). Grille
# refaite ici en DEUX series consecutives de 19 points (543-561 et
# 562-580), meme session continue, RG=136.56 (rga refait -- NE PAS
# reforcer 216.57, obsolete). Les deux series s'accordent a mieux que
# 1% aux D1>=8s (verifie via Check_lot_reproducibility.py) ; SERIES
# ci-dessous choisit laquelle sert de resultat officiel.
#
# Fit sur D1+AQ (pas D1 seul) -- lecon DS/AQ du 19-20/08.
# Ne couvre que le T1 de ce palier -- T2 traite dans
# T2_recovery_static_330K.py.
# ============================================================

# === CONFIGURATION -- only section to edit ===
# SERIES : "1" (exp543-561) ou "2" (exp562-580) -- les deux valides
# (accord <1% aux D1>=8s), "1" retenue par defaut.
SERIES = "1"
...
_EXP_SERIE_1 = list(range(543, 562))
_EXP_SERIE_2 = list(range(562, 581))
...
ZF_FACTOR = 1
PEAK_PPM_WINDOW = (-10, 15)   # same fix as 298K (26/08)
PPM_OUTLIER_THRESHOLD = 5.0   # ppm from the median -- flags a likely mispicked noise peak
FORCE_INCLUDE_D1 = [0.35, 1, 1.5, 6, 15]
                              # specifique aux mispicks de CETTE grille -- a reconstruire au vu
                              # des exclusions imprimees par le run, ne pas reutiliser telle quelle.
OUTPUT_NAME = rf"D:\Postdoc\Figures\T1_global_statique_330K_provisoire_serie{SERIES}"
                              # _provisoire : choix final (garder serie 1 seule, ou moyenner
                              # les deux) pas encore tranche -- renommer une fois decide.
# ================================================
...
if __name__ == "__main__":
    print(f"=== T1 series (static probe, 330K palier refait, serie {SERIES}) ===")
    ...
    ax.set_title(rf"$^7$Li T$_1$ recovery — static probe, 330K (refait 27-28/08, serie {SERIES}, RG=136.56)")
    ...
    export_agr(f"{OUTPUT_NAME}.agr", agr_series,
               ...
               xlog=True, title="7Li T1 recovery -- static probe, 330K (refait)")
